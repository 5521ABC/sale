...
<title>${siteName!""}|拍卖者管理-${title!""}</title>
                ...
                <form class="pull-right search-bar" method="get" action="list" role="form">
                  <div class="input-group">
                    <div class="input-group-btn">
                      ...
                      拍卖者 <span class="caret"></span>
                      </button>
                      <ul class="dropdown-menu">
                        <li> <a tabindex="-1" href="javascript:void(0)" data-field="title">拍卖者</a> </li>
                      </ul>
                    </div>
                    <input type="text" class="form-control" value="${name!""}" name="name" placeholder="请输入拍卖者名">
                      ...
                      <tr row-id="${item.id}" row-status="${item.auditStatus}">
                        ...
                        <td style="vertical-align:middle;">
                            <#if item.auditStatus == 1>
                                <button class="btn btn-label btn-primary" onclick="approvalAgree(${item.id},'PASS')">
                                    <label><i class="mdi mdi-checkbox-marked-circle-outline"></i></label>
                                    同意
                                </button>
                                <button class="btn btn-label btn-danger" onclick="approval(${item.id},'NOT_PASS')">
                                    <label><i class="mdi mdi-close"></i></label>
                                    拒绝
                                </button>
                            <#else>
                                 <button class="btn btn-label btn-primary" <#if item.auditStatus!=3>disabled</#if> onclick="approvalAgree(${item.id},'FROZEN')">
                                     <label><i class="mdi mdi-checkbox-marked-circle-outline"></i></label>
                                     冻结
                                 </button>
                                <button class="btn btn-label btn-danger" <#if item.auditStatus!=4>disabled</#if> onclick="approvalAgree(${item.id},'THAW')"  >
                                    <label><i class="mdi mdi-close"></i></label>
                                    解冻
                                </button>
                            </#if>
...
function edit(url){
    ...
    {
        showWarningMsg("该拍卖者不在冻结或可用状态无法编辑改拍卖者状态");
        return ;
    }
...
function approvalAgree(id,type){
    $.ajax({
        url:'examine',
        type:'post',
        data:{
        ...
        success:function (data) {
            if(data.code == 0){
                showSuccessMsg("状态变动成功",function () {

(function($) {
  ...
  $(".qty-box span").on('click', function() {
    var quantity = $(this).closest('.qty-box').find('input');
    debugger;
    var name=$(quantity).attr("name");
    var qtyVal = parseInt(quantity.val());
    var qtyVals=qtyVal+1;
    var maxValue=$(quantity).attr("data-max");
    if(name=="quantity"){
        if(qtyVals<=parseInt(maxValue)){
            if ($(this).hasClass('qty-add')) {
                quantity.val(qtyVal + 1);
            } else {
                return qtyVal > 1 ? quantity.val(qtyVal - 1) : 0;
            }
        }
    }else{
      if(qtyVal==parseInt(maxValue)){
          if ($(this).hasClass('qty-add')) {
              quantity.val(qtyVal + 1);
          }
      }else{

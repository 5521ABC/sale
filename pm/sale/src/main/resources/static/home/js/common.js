//显示成功信息
function showSuccessMsg(selector,msg,callback){
    $('.'+selector).popover({
        trigger: 'click',
		title:'成功信息',
		content:msg,
...
//显示警告信息
function showWarningMsg(selector,msg,callback){
...
function checkForm(formId,clazz,contentId){
	var flag = true;
	$("#"+formId).find(".required").each(function(i,e){
		if($(e).val() == ''){
			$("#"+contentId).text($(e).attr('tips'));
			$("."+clazz).show();
			flag = false;
			return false;
		}

...
public class AdminLoginInterceptor implements HandlerInterceptor{
	...
	public boolean  preHandle(HttpServletRequest request, HttpServletResponse response, Object handler){
		...
		if(!StringUtil.isAjax(request)){
			...
			List<Menu> auth = new ArrayList<>();
			if((Integer) type == LoginType.ADMINISTRATOR.getCode()){
				User user = (User)attribute;
				auth = user.getRole().getAuthorities();
			}else if((Integer) type == LoginType.ORGANIZATION.getCode()){
				Organization organization = (Organization)attribute;
				auth= organization.getRole().getAuthorities();
			}
			request.setAttribute("userTopMenus", MenuUtil.getTopMenus(auth));
			List<Menu> secondMenus = MenuUtil.getSecondMenus(auth);
			request.setAttribute("userSecondMenus", secondMenus);
			request.setAttribute("userThirdMenus", MenuUtil.getChildren(MenuUtil.getMenuIdByUrl(requestURI,secondMenus),auth));
		}
		return true;
	}
}
